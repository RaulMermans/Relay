...
Reports joins the navigation as a dedicated preview after a valid analysis. It minimizes dashboard chrome, provides Back to dashboard and Export PDF, and prints from the same report markup. When newer analysis makes the preview stale, it stays readable but export is disabled until the user selects Refresh report. Settings, Team, Billing, marketplace, Automations, and AI chat are not present.
