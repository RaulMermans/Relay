...
describe("connector contract", () => {
  ...
  it("registers implemented Shopify, Meta, and Google adapters without pretending live configuration exists", () => {
    expect(CONNECTOR_REGISTRY).toEqual([
      ...
      expect.objectContaining({ provider: "meta_ads", frameworkStatus: "ready", implementationStatus: "implemented", configured: false, connector: null }),
      expect.objectContaining({ provider: "google_ads", frameworkStatus: "ready", implementationStatus: "implemented", configured: false, connector: null }),
    ]);
