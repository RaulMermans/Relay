  ...
  return (
    <details className="memory-settings">
      ...
      <div className="memory-settings-grid">
        ...
        <section><p className="eyebrow">Reporting preferences</p><label><span>Cadence</span><select aria-label="Reporting cadence" value={client.reporting.cadence} onChange={(event) => onChange({ ...client, reporting: { ...client.reporting, cadence: event.target.value as "weekly" | "monthly" }, updatedAt: new Date().toISOString() })}><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><fieldset><legend>Future report sections</legend>{(Object.keys(SECTION_LABELS) as ReportSection[]).map((section) => <label key={section}><input type="checkbox" checked={client.reportPreferences.sections.includes(section)} onChange={() => toggleSection(section)} />{SECTION_LABELS[section]}</label>)}</fieldset></section>
      </div>
